...
describe "A user can reset password" do
  ...
  it "User MUST enter correct email to get Twillo 2FA code", :vcr do
    code = rand(100000...999999)
    ...
    fill_in "password[email]", with: "test@test.com"
    click_on "Request confirmation code"

    expect(page).to have_content("You didnt enter the right email")
  end

  it "User MUST enter the right email to reset their password", :vcr do
    code = rand(100000...999999)
    ...
    fill_in 'password[twilio_response_code]', with: new_code
    fill_in "password[password]", with: "Turing1510"
    fill_in "password[password_confirmation]", with: "Turing151"
    click_on "Update Password"

    expect(page).to have_content("Your passswords do not match. Please try again.")
  end
end
